...
fn main() {
    ...
    for chunk in records.chunks(4) {
        ...
        ins.push(
            ComputeBudgetInstruction::set_compute_unit_price(10000000) // 10 lamports
        );
        ...
        // println!("{:?}", simulation);
        let send = rpc_client.send_and_confirm_transaction_with_spinner_and_commitment(
            &tx,
        ...
        for rec in chunk {
            wtr.serialize((&rec.address, &rec.amount, tx_hash.to_string()))
                .unwrap();
        }
        wtr.flush().unwrap();
