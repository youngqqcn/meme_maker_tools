...
export type CreateMarketInput = {
    ...
    marketKeyPair: web3.Keypair | undefined; // 可以自定义
};
...
export class BaseRay {
    ...
    constructor(input: BaseRayInput) {
        this.connection = new Connection(input.rpcEndpointUrl, {
        ...
        this.pools = new Map();
        if (input.rpcEndpointUrl.includes("devnet")) {
    ...
    private addPoolKeys(
        poolInfo: LiquidityAssociatedPoolKeys,
        marketState: any
    ) {
        const {
            authority,
    ...
    async removeLiquidity(
        ...
        const lpTokenAccountInfo = await this.connection.getAccountInfo(
            lpTokenAccount
        );
    ...
    async createPool(input: CreatePoolInput, user: web3.PublicKey) {
        ...
        if (
            input.baseMint.toBase58() != NATIVE_MINT.toBase58() &&
            !userBaseAtaInfo
        )
        ...
        const createPoolIxs = (
            await Liquidity.makeCreatePoolV4InstructionV2Simple({
                ...
                ownerInfo: {
                    ...
                    tokenAccounts: [
                        {
                            accountInfo: SPL_ACCOUNT_LAYOUT.decode(
                                userBaseAtaInfo!.data
                            ),
    ...
    > {
        ...
        const marketAccounts = {
            market: input.marketKeyPair ?? Keypair.generate(), // 如果自定义了，则使用自定义的keypair; 否则临时生成
        ...
        try {
            baseMintDecimals = MintLayout.decode(
                baseMintAccountInfo.data
            ).decimals;
            quoteMintDecimals = MintLayout.decode(
                quoteMintAccountInfo.data
            ).decimals;
        } catch (error) {
            return { Err: "Invalid token address! Token not found" };
        }

        console.log("baseMintDecimals: ", baseMintDecimals);
        console.log("quoteMintDecimals: ", quoteMintDecimals);
        ...
        marketInstructions.push(
            DexInstructions.initializeMarket({
                ...
                baseMint,
                quoteMint,
                baseLotSize: baseLotSize, // TODO: 这里写死了
                quoteLotSize: quoteLotSize, // TODO: 这里写死了
    ...
    async computeAnotherAmount({
        ...
        if (poolAccountInfo.data.length == LIQUIDITY_STATE_LAYOUT_V4.span) {
            ...
            poolAccountInfo.data.length == LIQUIDITY_STATE_LAYOUT_V5.span
        ) {
        ...
        const baseReserve = new BN(
            Number(
                AccountLayout.decode(baseVAccountInfo.data).amount.toString()
            )
        );
        const quoteReserve = new BN(
            Number(
                AccountLayout.decode(quoteVAccountInfo.data).amount.toString()
            )
        );
