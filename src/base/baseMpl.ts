...
import {
    ...
    Sft,
} from "@metaplex-foundation/js/dist/types";
// import {
...
export class BaseMpl {
    ...
    setUpCallBack = (ixs: TransactionInstruction[], signs: Keypair[]) => {
        if (ixs) {
    ...
    async createToken(
        ...
        try {
            ...
            let { ixs: mintIxs, mintKeypair: _mintKeypair } =
                await baseSpl.createToken({
                    decimal,
                    mintAuthority: user,
                    mintingInfo: { tokenAmount: mintAmount },
                    mintKeypair,
                });
            ixs.push(...mintIxs);
            ...
            const txBuilder = await this.metaplex
                .nfts()
                .builders()
                .create(input);
            ...
            if (opts.revokeAuthorities) {
                ixs.push(
                    this.baseSpl.revokeAuthority({
                        ...
                        currentAuthority: this.provider.publicKey,
                        mint,
                    })
                );
                ixs.push(
            ...
            const res = await this.provider
                .sendAndConfirm(tx, [mintKeypair], { maxRetries: 20 })
                .catch(async () => {
                    await sleep(2_000);
                    return this.provider
                        .sendAndConfirm(tx, [mintKeypair!], { maxRetries: 20 })
                        .catch((createTokenSendAndConfirmError) => {
    ...
    async burn(input: BurnInput) {
        ...
        if (!amount) {
            ...
            if (!decimal) decimal = (await this.baseSpl.getMint(mint)).decimals;
        ...
        const ixs = this.metaplex
            ...
            .delete({
                mintAddress: mint,
                amount: {
                    basisPoints: new BN(amount) as any,
                    currency: {
                        decimals: decimal,
                        namespace: "spl-token",
                        symbol: "",
